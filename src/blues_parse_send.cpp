...
/**
 * @brief Create a sensor.qo note with the CayenneLPP formatted payload
 *        Sends the note to the NoteCard with "sync" request for immediate sending
 * 
 * @param data Pointer to the uint8_t array with the payload
 * @param data_len length of the payload
 * @return true if the note could be created and sent
 * @return false if the note could not be created or could not be sent
 */
bool blues_parse_send(uint8_t *data, uint16_t data_len)
{
	...
	if (body != NULL)
	{
		...
		// 		// float_val1 = (float)((int16_t)data[current_byte_idx + 0] << 8 | (int16_t)data[current_byte_idx + 1]) / value_divider[sens_idx];
		// 		signed_val1 = (int16_t)data[current_byte_idx + 0] << 8 | (int16_t)data[current_byte_idx + 1];
		// 		float_val1 = (float)signed_val1 / value_divider[sens_idx];
		// 		current_byte_idx += 2;
		// 		JAddNumberToObject(sec_lvl, "X", float_val1);
		// 		// float_val2 = (float)((int16_t)data[current_byte_idx + 0] << 8 | (int16_t)data[current_byte_idx + 1]) / value_divider[sens_idx];
		// 		signed_val1 = (int16_t)data[current_byte_idx + 0] << 8 | (int16_t)data[current_byte_idx + 1];
		// 		float_val2 = (float)signed_val1 / value_divider[sens_idx];
		// 		current_byte_idx += 2;
		// 		JAddNumberToObject(sec_lvl, "Y", float_val2);
		// 		// float_val3 = (float)((int16_t)data[current_byte_idx + 0] << 8 | (int16_t)data[current_byte_idx + 1]) / value_divider[sens_idx];
		// 		signed_val1 = (int16_t)data[current_byte_idx + 0] << 8 | (int16_t)data[current_byte_idx + 1];
		// 		float_val3 = (float)signed_val1 / value_divider[sens_idx];
		// 		current_byte_idx += 2;
		...
		// 		float_val1 = (float)((int32_t)data[current_byte_idx + 0] << 16 | (int32_t)data[current_byte_idx + 1] << 8 | (int32_t)data[current_byte_idx + 2]) / 10000.0;
		// 		current_byte_idx += 3;
		// 		JAddNumberToObject(sec_lvl, "Lat", float_val1);
		// 		float_val2 = (float)((int32_t)data[current_byte_idx + 0] << 16 | (int32_t)data[current_byte_idx + 1] << 8 | (int32_t)data[current_byte_idx + 2]) / 10000.0;
		// 		current_byte_idx += 3;
		// 		JAddNumberToObject(sec_lvl, "Lng", float_val2);
		// 		float_val3 = (float)((int32_t)data[current_byte_idx + 0] << 16 | (int32_t)data[current_byte_idx + 1] << 8 | (int32_t)data[current_byte_idx + 2]) / 100.0;
		// 		current_byte_idx += 3;
		// 		JAddNumberToObject(sec_lvl, "Alt", float_val3);
		// 		MYLOG("PARSE", "lat %.4f lng %.4f alt %.4f", float_val1, float_val2, float_val3);

		// 		JAddNumberToObject(body, "alt_10", float_val3);
		// 		JAddNumberToObject(body, "lat_10", float_val1);
		// 		JAddNumberToObject(body, "lng_10", float_val2);
		// 		break;
		// 	case 137:
		...
		// 		// float_val1 = (float)((int32_t)data[current_byte_idx + 0] << 24 | (int32_t)data[current_byte_idx + 1] << 16 | (int32_t)data[current_byte_idx + 2] << 8 | (int32_t)data[current_byte_idx + 3]) / 1000000.0;
		// 		signed_val1 = (int32_t)data[current_byte_idx + 0] << 24 | (int32_t)data[current_byte_idx + 1] << 16 | (int32_t)data[current_byte_idx + 2] << 8 | (int32_t)data[current_byte_idx + 3];
		// 		float_val1 = (float)signed_val1 / 1000000.0;
		// 		current_byte_idx += 4;
		// 		JAddNumberToObject(sec_lvl, "Lat", float_val1);
		// 		// float_val2 = (float)((int32_t)data[current_byte_idx + 0] << 24 | (int32_t)data[current_byte_idx + 1] << 16 | (int32_t)data[current_byte_idx + 2] << 8 | (int32_t)data[current_byte_idx + 3]) / 1000000.0;
		// 		signed_val1 = (int32_t)data[current_byte_idx + 0] << 24 | (int32_t)data[current_byte_idx + 1] << 16 | (int32_t)data[current_byte_idx + 2] << 8 | (int32_t)data[current_byte_idx + 3];
		// 		float_val2 = (float)signed_val1 / 1000000.0;
		// 		current_byte_idx += 4;
		// 		JAddNumberToObject(sec_lvl, "Lng", float_val2);
		// 		// float_val3 = (float)((int32_t)data[current_byte_idx + 0] << 16 | (int32_t)data[current_byte_idx + 1] << 8 | (int32_t)data[current_byte_idx + 2]) / 100.0;
		// 		signed_val1 = (int32_t)data[current_byte_idx + 0] << 16 | (int32_t)data[current_byte_idx + 1] << 8 | (int32_t)data[current_byte_idx + 2];
		// 		float_val3 = (float)signed_val1 / 100.0;
		// 		current_byte_idx += 3;
		// 		JAddNumberToObject(sec_lvl, "Alt", float_val3);
		// 		MYLOG("PARSE", "lat %.6f lng %.6f alt %.6f", float_val1, float_val2, float_val3);
		...
		// 		sens_full_name = value_name[sens_idx] + "_" + String(sens_num);
		// 		// JAddStringToObject(body, sens_full_name.c_str(), rounding);
